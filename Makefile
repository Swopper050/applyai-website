...
prod_db_backup: ## Makes a backup from the prod database using ssh
	ssh applyai docker exec $(CONTAINER_NAME) mariadb-dump --all-databases -uroot -p$(MYSQL_ROOT_PASSWORD) > ./prod_db_backup.sql

load_prod_db_backup: ## Loads the prod database backup
	docker exec -i applyai-mariadb sh -c 'exec mariadb -uroot -papplyai_root_password' < ./prod_db_backup.sql
